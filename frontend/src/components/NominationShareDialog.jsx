...
const NominationShareDialog = ({ open, onClose, workshopId, workshopTitle }) => {
  ...
  const txt = {
    sv: {
      ...
      shareSystem: 'Dela...',
      createLinks: 'Skapa länkar',
      noRecipients: 'Lägg till minst en mottagare',
      linksCopied: 'Länkar kopierade!',
      linksCreated: 'Länkar skapade!',
      sendNow: 'Skicka nu',
      results: 'Resultat',
      status: {
        created: 'Skapad',
    ...
    en: {
      title: 'Share nomination link',
      subtitle: 'Invite multiple people to nominate participants',
  ...
  const sendViaWhatsApp = (recipient, link) => {
    const text = encodeURIComponent(`${senderInfo.name} ${language === 'ar' ? 'يدعوك للترشيح في' : 'invites you to nominate for'} ${workshopTitle}:\n\n${message ? message + '\n\n' : ''}${link}`);
    const phone = recipient.recipient_contact.replace(/[^0-9]/g, '');
