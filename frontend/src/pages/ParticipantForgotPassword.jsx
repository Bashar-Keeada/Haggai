...
const ParticipantForgotPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${BACKEND_URL}/api/participants/forgot-password`, {
        method: 'POST',
      ...
      <div className={`min-h-screen bg-gradient-to-br from-blue-50 via-cream-50 to-cream-100 flex items-center justify-center p-4 ${isRTL ? 'rtl' : 'ltr'}`}>
        <Card className="w-full max-w-md border-0 shadow-2xl">
          <CardContent className="p-8 text-center">
            ...
            <Link to="/deltagare/login">
              <Button className="bg-[#0891B2]">{t.backToLogin}</Button>
            </Link>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="w-full max-w-md border-0 shadow-2xl">
        ...
        <CardContent className="p-6">
          ...
          <div className="mt-6 text-center">
            <Link to="/deltagare/login" className="text-sm text-haggai hover:underline flex items-center justify-center gap-1">
