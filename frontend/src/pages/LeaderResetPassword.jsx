...
const LeaderResetPassword = () => {
  ...
  const txt = {
    ...
    ar: {
      ...
      successMessage: 'يمكنك الآن تسجيل الدخول بكلمة المرور الجديدة',
      goToLogin: 'الذهاب لتسجيل الدخول',
      errorMessage: 'تعذر إعادة تعيين كلمة المرور'
    }
  };
  ...
  const validateToken = async () => {
    try {
      const response = await fetch(`${BACKEND_URL}/api/leaders/validate-reset-token/${token}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${BACKEND_URL}/api/leaders/reset-password`, {
        method: 'POST',
  ...
  if (validating) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-haggai-50 via-cream-50 to-cream-100 flex items-center justify-center">
      ...
      <div className={`min-h-screen bg-gradient-to-br from-haggai-50 via-cream-50 to-cream-100 flex items-center justify-center p-4 ${isRTL ? 'rtl' : 'ltr'}`}>
        <Card className="w-full max-w-md border-0 shadow-2xl">
          <CardContent className="p-8 text-center">
            ...
            <Button onClick={() => navigate('/ledare/login')} className="mt-4 bg-haggai">
              {t.goToLogin}
            </Button>
          </CardContent>
        </Card>
      ...
      <div className={`min-h-screen bg-gradient-to-br from-haggai-50 via-cream-50 to-cream-100 flex items-center justify-center p-4 ${isRTL ? 'rtl' : 'ltr'}`}>
        <Card className="w-full max-w-md border-0 shadow-2xl">
          <CardContent className="p-8 text-center">
            ...
            <Button onClick={() => navigate('/ledare/login')} className="bg-haggai">
              {t.goToLogin}
            </Button>
          </CardContent>
        </Card>
...
export default LeaderResetPassword;
