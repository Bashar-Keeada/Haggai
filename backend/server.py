...
async def send_nomination_confirmation_to_nominator(nomination: Nomination):
    """Send confirmation email to the person who submitted the nomination - Always in Arabic"""
    html_content = f"""
    <!DOCTYPE html>
    <html dir="rtl">
    <head><meta charset="UTF-8"></head>
    <body style="font-family: Arial, sans-serif; line-height: 1.8; color: #333; max-width: 600px; margin: 0 auto; padding: 20px; direction: rtl; text-align: right;">
        <div style="background: linear-gradient(135deg, #014D73 0%, #012d44 100%); padding: 30px; border-radius: 10px 10px 0 0; text-align: center;">
            <h1 style="color: white; margin: 0; font-size: 24px;">✅ تم استلام الترشيح</h1>
        </div>
        
        <div style="background: #f9f9f9; padding: 30px; border: 1px solid #ddd; border-radius: 0 0 10px 10px;">
            <p>مرحباً <strong>{nomination.nominator_name}</strong>،</p>
            
            <p>شكراً لترشيحك <strong>{nomination.nominee_name}</strong> لـ <strong>{nomination.event_title}</strong>!</p>
            
            <div style="background: #e3f2fd; padding: 20px; border-radius: 8px; border-right: 4px solid #014D73; margin: 20px 0;">
                <p style="margin: 0; font-weight: bold;">ماذا يحدث الآن؟</p>
                <ol style="margin: 10px 0 0 0; padding-right: 20px;">
                    <li>سيتم مراجعة ترشيحك من قبل فريقنا</li>
                    <li>إذا تمت الموافقة على الترشيح، سيتم إرسال دعوة إلى {nomination.nominee_name}</li>
                    <li>ستتلقى إشعاراً عند معالجة الترشيح</li>
                </ol>
            </div>
    ...
    try:
        await asyncio.to_thread(resend.Emails.send, {
            ...
            "subject": f"✅ تم استلام ترشيحك لـ {nomination.nominee_name}",
            "html": html_content
        })
    except Exception as e:
...
async def send_nomination_approved_to_nominator(nomination: Nomination):
    """Notify nominator that their nomination was approved - Always in Arabic"""
    html_content = f"""
    <!DOCTYPE html>
    <html dir="rtl">
    <head><meta charset="UTF-8"></head>
    <body style="font-family: Arial, sans-serif; line-height: 1.8; color: #333; max-width: 600px; margin: 0 auto; padding: 20px; direction: rtl; text-align: right;">
        <div style="background: linear-gradient(135deg, #2e7d32 0%, #1b5e20 100%); padding: 30px; border-radius: 10px 10px 0 0; text-align: center;">
            <h1 style="color: white; margin: 0; font-size: 24px;">✅ تمت الموافقة على الترشيح!</h1>
        </div>
        
        <div style="background: #f9f9f9; padding: 30px; border: 1px solid #ddd; border-radius: 0 0 10px 10px;">
            <p>مرحباً <strong>{nomination.nominator_name}</strong>،</p>
            
            <p>أخبار سارة! تمت الموافقة على ترشيحك لـ <strong>{nomination.nominee_name}</strong> للمشاركة في <strong>{nomination.event_title}</strong>.</p>
            
            <div style="background: #e8f5e9; padding: 20px; border-radius: 8px; border-right: 4px solid #2e7d32; margin: 20px 0;">
                <p style="margin: 0;">لقد أرسلنا الآن دعوة إلى {nomination.nominee_name} تتضمن معلومات عن البرنامج ورابط للتسجيل.</p>
            </div>
            
            <p>شكراً لمساعدتك في تحديد وتشجيع القادة المستقبليين!</p>
    ...
    try:
        await asyncio.to_thread(resend.Emails.send, {
            "from": SENDER_EMAIL,
            "to": [nomination.nominator_email],
            "subject": f"✅ تمت الموافقة على ترشيحك لـ {nomination.nominee_name}!",
            "html": html_content
        })
    except Exception as e:
...
async def notify_participants_agenda_published(workshop_id: str, workshop: dict):
    """Send email to all accepted participants AND leaders assigned to this workshop's agenda"""
    ...
    # Get leaders/facilitators assigned to ANY session in this workshop's agenda
    agenda = await db.workshop_agendas.find_one({"workshop_id": workshop_id}, {"_id": 0})
    leader_emails = set()
    
    if agenda:
        # Loop through ALL days and ALL sessions in the agenda
        for day in agenda.get("days", []):
            for session in day.get("sessions", []):
...
async def create_leader_invitation(input: LeaderInvitationCreate):
    ...
    base_url = os.environ.get('FRONTEND_URL', 'https://haggai-portal-2.preview.emergentagent.com')
    ...
    email_texts = {
        ...
        "ar": {
            "subject": "🎉 أنت مدعو كميسر / مدرب - ورشة عمل هاجاي السويد",
            "header": "أنت مدعو كميسر / مدرب!",
            "greeting": f"مرحباً <strong>{input.name}</strong>،",
            "intro": "يسعدنا أن نبلغك أنك قد تمت دعوتك للمشاركة كـ<strong>ميسر / مدرب</strong> في ورشة عمل هاجاي السويد القادمة! 🙌",
